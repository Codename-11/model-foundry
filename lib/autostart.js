...
const WINDOWS_STARTUP_FILE = 'model-foundry-autostart.cmd'
const LINUX_SERVICE_FILE = 'model-foundry.service'
const LEGACY_LINUX_SERVICE_FILE = 'modelrelay.service'
const MACOS_AGENT_FILE = 'io.modelfoundry.autostart.plist'
const MACOS_AGENT_LABEL = 'io.modelfoundry.autostart'
...
function getPaths(platform = process.platform) {
  ...
  if (platform === 'linux') {
    return {
      ...
      legacyFilePath: join(homedir(), '.config', 'systemd', 'user', LEGACY_LINUX_SERVICE_FILE),
      legacyUnitName: LEGACY_LINUX_SERVICE_FILE,
    }
  }
...
function resolveLinuxServicePath(paths) {
  if (existsSync(paths.filePath)) return paths.filePath
  if (paths.legacyFilePath && existsSync(paths.legacyFilePath)) return paths.legacyFilePath
  return paths.filePath
}

function resolveLinuxUnitName(paths) {
  if (existsSync(paths.filePath)) return paths.unitName
  if (paths.legacyUnitName && paths.legacyFilePath && existsSync(paths.legacyFilePath)) return paths.legacyUnitName
  return paths.unitName
}
...
function linuxUnitFile(execPath, nodePath) {
  ...
  return [
    '[Unit]',
    'Description=model-foundry autostart',
...
function macosPlist() {
  const logPath = join(homedir(), '.model-foundry-autostart.log')
  const errorPath = join(homedir(), '.model-foundry-autostart.err.log')
...
function windowsStartupScript() {
  return ['@echo off', 'start "" /min model-foundry', ''].join('\r\n')
}
...
export function getAutostartStatus(platform = process.platform) {
  ...
  if (platform === 'linux') {
    const servicePath = resolveLinuxServicePath(paths)
    const unitName = resolveLinuxUnitName(paths)
    const configured = existsSync(servicePath)
    const enabledResult = run('systemctl', ['--user', 'is-enabled', unitName])
    const activeResult = run('systemctl', ['--user', 'is-active', unitName])
...
export function startAutostart(platform = process.platform) {
  ...
  const servicePath = platform === 'linux' ? resolveLinuxServicePath(paths) : paths.filePath
  if (!existsSync(servicePath)) {
    return {
      ok: false,
      supported: true,
      message: 'Autostart is not installed. Run: model-foundry install --autostart',
    }
  }

  if (platform === 'win32') {
    const startResult = run('cmd.exe', ['/d', '/s', '/c', 'start "" /min model-foundry'])
    ...
    return {
      ok: true,
      supported: true,
      message: 'Autostart target triggered. model-foundry should start shortly.',
    }
  }

  if (platform === 'linux') {
    const unitName = resolveLinuxUnitName(paths)
    const startResult = run('systemctl', ['--user', 'start', unitName])
    ...
    return {
      ok: true,
      supported: true,
      message: `Autostart service started: ${unitName}`,
    }
  }
...
export function stopAutostart(platform = process.platform) {
  ...
  if (platform === 'win32') {
    const script = [
      ...
      "  $_.CommandLine -and $_.CommandLine -match 'model-foundry' -and $_.ProcessId -ne $currentPid",
      '}',
    ...
    if (stopResult.error?.code === 'ENOENT') {
      return {
        ok: false,
        supported: true,
        message: 'Autostart is configured, but PowerShell was not found to stop background model-foundry processes.',
      }
    }
    ...
    return {
      ...
      supported: true,
      configured: true,
      message: 'Stopped background model-foundry process(es).',
    }
  }

  if (platform === 'linux') {
    const unitName = resolveLinuxUnitName(paths)
    const stopResult = run('systemctl', ['--user', 'stop', unitName])
    if (stopResult.error?.code === 'ENOENT') {
      return {
        ok: false,
        supported: true,
    ...
    return {
      ...
      supported: true,
      configured: true,
      message: `Autostart service stopped: ${unitName}`,
    }
  }
...
export function uninstallAutostart(platform = process.platform) {
  ...
  if (platform === 'win32') {
    ...
    if (paths.legacyFilePath && existsSync(paths.legacyFilePath)) rmSync(paths.legacyFilePath)
    return {
      ok: true,
      supported: true,
  ...
  if (platform === 'linux') {
    const unitName = resolveLinuxUnitName(paths)
    const disableResult = run('systemctl', ['--user', 'disable', '--now', unitName])

/**
 * @file lib/config.js
 * @description JSON config management for ModelFoundry multi-provider support.
 *
 * 📖 This module manages ~/.model-foundry.json, the config file that
 *    stores API keys and per-provider enabled/disabled state for all providers
 *    (NVIDIA NIM, Groq, Cerebras, etc.).
 *
 * 📖 Config file location: ~/.model-foundry.json
 ...
 *
 * @functions
 *   → loadConfig() — Read ~/.model-foundry.json
 *   → saveConfig(config) — Write config to ~/.model-foundry.json with 0o600 permissions
 ...
 *
 * @see bin/modelrelay.js — legacy CLI entrypoint that uses these functions
 ...
 *
 * 📖 Fallback chain:
 *   1. Try to read ~/.model-foundry.json
 *   2. If missing or invalid, return an empty default config
 *
 * @returns {{ apiKeys: Record<string,string>, providers: Record<string,{enabled:boolean, catalogVisible?: boolean}>, bannedModels: string[], autoUpdate: { enabled: boolean, intervalHours: number, lastCheckAt: string|null, lastUpdateAt: string|null, lastVersionApplied: string|null, lastError: string|null }, minSweScore: number|null, excludedProviders: string[] }}
 */
...
/**
 * 📖 saveConfig: Write the config object to ~/.model-foundry.json.
 *
 ...
 *
 * 📖 Fresh installs should not implicitly activate every provider.
 * 📖 Providers become active once they are explicitly enabled or already have
 *    provider-specific configuration/env credentials present.
 *

 ...
 */
export function writeConfigFiles(config, primaryPath = CONFIG_PATH, legacyPath = LEGACY_CONFIG_PATH) {
  const normalized = normalizeConfigShape(config)
  const serialized = JSON.stringify(normalized, null, 2)

  writeFileSync(primaryPath, serialized, { mode: 0o600 })
  ...
  return {
    primaryPath,
    legacyPath,
    primaryMode: statSync(primaryPath).mode,
    legacyMode: statSync(legacyPath).mode,
    config: normalized,
  }
}

export function saveConfig(config) {
  return writeConfigFiles(config)
}

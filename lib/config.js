...
// 📖 Canonical JSON config path — stores all providers' API keys + enabled state
export const CONFIG_PATH = join(homedir(), '.model-foundry.json')
export const LEGACY_CONFIG_PATH = join(homedir(), '.modelrelay.json')
...
const HERMES_PROXY_INSTANCE_ID = 'hermes-proxy'
export const HERMES_PROXY_INSTANCE_KEY = `${OPENAI_COMPATIBLE_INSTANCE_PREFIX}${HERMES_PROXY_INSTANCE_ID}`
const DEFAULT_HERMES_PROXY_BASE_URL = 'http://127.0.0.1:8648/v1'
const HERMES_PROXY_API_KEY_ENV_VAR = 'MODELFOUNDRY_HERMES_PROXY_API_KEY'
const HERMES_PROXY_BASE_URL_ENV_VAR = 'MODELFOUNDRY_HERMES_PROXY_BASE_URL'
const HERMES_PROXY_MODEL_ENV_VAR = 'MODELFOUNDRY_HERMES_PROXY_MODEL'
...
export function isHermesProxyProviderKey(providerKey) {
  return providerKey === HERMES_PROXY_INSTANCE_KEY
}
...
export function buildHermesProxyEndpointPreset(overrides = {}) {
  const configuredBaseUrl = typeof process !== 'undefined' ? process.env[HERMES_PROXY_BASE_URL_ENV_VAR] : ''
  const configuredApiKey = typeof process !== 'undefined' ? process.env[HERMES_PROXY_API_KEY_ENV_VAR] : ''
  const configuredModel = typeof process !== 'undefined' ? process.env[HERMES_PROXY_MODEL_ENV_VAR] : ''
...
const ENV_VARS = {
  ...
  [HERMES_PROXY_INSTANCE_KEY]: HERMES_PROXY_API_KEY_ENV_VAR,
...
const PROVIDER_BASE_URL_ENV_VARS = {
  ...
  [HERMES_PROXY_INSTANCE_KEY]: HERMES_PROXY_BASE_URL_ENV_VAR,
...
const PROVIDER_MODEL_ID_ENV_VARS = {
  ...
  openai: 'OPENAI_MODEL',
  gemini: 'GEMINI_MODEL',
  [HERMES_PROXY_INSTANCE_KEY]: HERMES_PROXY_MODEL_ENV_VAR,
...
export function loadConfig() {
  ...
  const legacy = _readConfigFile(LEGACY_CONFIG_PATH)
...
export function saveConfig(config) {
  try {
    ...
    writeFileSync(CONFIG_PATH, serialized, { mode: 0o600 })

    // 📖 Legacy compatibility mirror for older installs/scripts that still read ~/.modelrelay.json
    try {
      writeFileSync(LEGACY_CONFIG_PATH, serialized, { mode: 0o600 })
    } catch {
      // Ignore legacy mirror failures; canonical config already saved.
    }
  } catch {
...
export function getApiKey(config, providerKey) {
  ...
  // 📖 gemini convenience: also honor GOOGLE_API_KEY if GEMINI_API_KEY is unset
  if (providerKey === 'gemini' && process.env.GOOGLE_API_KEY) {
    return normalizeSecret(process.env.GOOGLE_API_KEY);
  }
...
export function isProviderEnabled(config, providerKey) {
  ...
  if (!providerConfig) {
    ...
    // 📖 Axiom-Labs: fresh installs should not implicitly activate every provider.
    // 📖 A provider is active by default only when it has credentials or routing
    //    config present (env var, API key, base URL, or model ID).
    return Boolean(
...
export function getProviderCatalogVisible(config, providerKey) {
  const providerConfig = config?.providers?.[providerKey]
  if (!providerConfig) return true
  return providerConfig.catalogVisible !== false
}

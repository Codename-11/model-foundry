const PROVIDER_META = {
  ...
  'openai-compatible:hermes-proxy': {
    category: 'advanced',
    recommendation: 'Hermes OAuth passthrough',
    summary: 'Use Hermes-managed OAuth subscriptions through the local OpenAI-compatible Hermes Proxy.',
    quickstartLabel: 'Hermes Proxy (OAuth passthrough)',
    quickstartDescription: 'Route through Hermes auth instead of copying provider API keys into ModelFoundry.',
  },
  anthropic: {
    category: 'frontier',
...
const PROVIDER_SETUP_META = {
  ...
  'openai-compatible:hermes-proxy': {
    defaultBaseUrl: 'http://127.0.0.1:8648/v1',
    defaultModelId: '',
    authLabel: 'Hermes OAuth passthrough',
    costLabel: 'Uses Hermes auth',
    setupHint: 'Uses the local Hermes Proxy for raw OpenAI-compatible inference. The bearer value is only a local placeholder; Hermes attaches the real OAuth credential. Leave model blank and refresh models to discover live routes.',
  },
  opencode: {

...
const NPM_LATEST_URL = 'https://registry.npmjs.org/model-foundry/latest'
const __dirname = dirname(fileURLToPath(import.meta.url))
const PROJECT_ROOT = join(__dirname, '..')
const CANONICAL_ENV_PREFIX = 'MODEL_FOUNDRY'
const LEGACY_ENV_PREFIX = 'MODELRELAY'

function readCompatEnv(name) {
  const canonical = process.env[`${CANONICAL_ENV_PREFIX}_${name}`]
  if (typeof canonical === 'string' && canonical.trim()) return canonical

  const legacy = process.env[`${LEGACY_ENV_PREFIX}_${name}`]
...
export function buildWindowsPostUpdateRestartCommand(autostartConfigured = false) {
  return autostartConfigured
    ? 'timeout /t 2 /nobreak && model-foundry start --autostart'
    : 'timeout /t 2 /nobreak && model-foundry'
}

export function getLocalUpdateTarballPath() {
  const value = readCompatEnv('UPDATE_TARBALL')
...
export function getForcedUpdateVersion() {
  const value = readCompatEnv('FORCE_UPDATE_VERSION')
  if (!value || !parseVersionParts(value)) return null
  return value.trim()
}

export function getLocalUpdateVersion() {
  const envVersion = readCompatEnv('UPDATE_VERSION')
  if (envVersion && parseVersionParts(envVersion)) {
  ...
  const match = basename(tarballPath).match(/model-foundry-(\d+\.\d+\.\d+)\.tgz$/i)
...
export function buildNpmInstallInvocation(target = 'latest', platform = process.platform) {
  ...
  const packageRef = localTarball || `model-foundry@${target}`
...
function runNpmUpdate(target = 'latest') {
  ...
  if (result.status !== 0) {
    ...
    if (details.includes('EACCES') || details.includes('EPERM')) {
      return {
        ok: false,
        message: `Permission denied during ${updateType} update. Please run "npm install -g model-foundry" with sudo/Administrator privileges.`,
      }
    }
    return {
      ok: false,
  ...
  return {
    ...
    message: localTarball
      ? `Updated model-foundry from local package${getLocalUpdateVersion() ? ` (v${getLocalUpdateVersion()})` : ''}.`
      : `Updated model-foundry to ${target === 'latest' ? 'latest version' : `v${target}`} (${updateType}).`,
  }
}

...
async function configurePicoClaw(port) {
  ...
  // Add to model_list, replacing any existing ModelFoundry entry
  if (!config.model_list) config.model_list = [];
  const existingIdx = config.model_list.findIndex(m => m.model_name === 'model-foundry');
  if (existingIdx !== -1) {
  ...
  if (!config.agents) config.agents = {};
  if (!config.agents.defaults) config.agents.defaults = {};
  if (!config.agents.defaults.model_name || config.agents.defaults.model_name === '') config.agents.defaults.model_name = 'model-foundry';

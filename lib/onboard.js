...
import { loadConfig, saveConfig, getApiKeyPool, getMaxTurns, buildHermesProxyEndpointPreset, upsertOpenAICompatibleEndpoint } from './config.js'
...
const CONFIGURABLE_CHAT_COMPLETIONS_KEYS = new Set([
  'openai',
  'anthropic',
  'gemini',
  'openai-compatible',
...
export function buildHermesAgentProviderConfig(port) {
  return {
...
// Backward-compatible export for older tests/scripts. New setup copy should refer to Hermes Agent.
export function buildOpenClawProviderConfig(port) {
  return buildHermesAgentProviderConfig(port)
}

export function applyHermesProxyEndpointPreset(config, overrides = {}) {
  return upsertOpenAICompatibleEndpoint(config, buildHermesProxyEndpointPreset(overrides))
}
...
function printIntegrationSnippets(port) {
  const hermesCommands = [
    '# Hermes Agent custom-provider lane for ModelFoundry',
  ...
  console.log('\nHermes Agent quick config (uses `hermes config set`; see https://hermes-agent.nousresearch.com/docs/user-guide/configuration):\n')
  console.log(hermesCommands)
...
export async function runOnboard(port = 7352) {
  ...
  for (const providerKey of PROVIDER_ORDER) {
    ...
    if (CONFIGURABLE_CHAT_COMPLETIONS_KEYS.has(providerKey)) {
      console.log('  note: configure the provider base URL and model ID after the API key prompt')
    }
    if (providerKey === 'anthropic') {
      console.log('  note: paid Claude frontier lane (separate from open-router pool)')
    }
    if (providerKey === 'openai') {
      console.log('  note: paid GPT/Codex frontier lane (separate from open-router pool)')
    }
    if (providerKey === 'gemini') {
      console.log('  note: paid Gemini frontier lane; falls back to GOOGLE_API_KEY if GEMINI_API_KEY unset')
    }
    if (providerKey === 'kiro') {
  ...
  const hermesProxyAnswer = await rl.question('Configure local Hermes Proxy endpoint? [Y/n]: ')
  const doHermesProxy = parseYesNo(hermesProxyAnswer, true)
  if (doHermesProxy) {
    const instanceKey = applyHermesProxyEndpointPreset(config)
    console.log(`Hermes Proxy endpoint configured: ${instanceKey}`)
    console.log('Start Hermes Proxy separately when needed: hermes proxy start --provider auto --host 127.0.0.1 --port 8648')
  }
  ...
  if (multiAccountProviders.length > 0) {
    ...
    if (hasSingleKeys) {
      ...
      console.log('  model-foundry config add-key <provider> <key2>')
      console.log('  model-foundry config set-maxturns <provider> 20')
    }
  }

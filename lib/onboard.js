...
import { getProviderMeta, getProviderSetupMeta, getQuickstartProviderChoices, parseQuickstartProviderChoice } from './providerMeta.js'
...
function isConfigurableModelProvider(providerKey) {
  const meta = getProviderSetupMeta(providerKey)
  return meta.defaultBaseUrl !== null || meta.defaultModelId !== null || providerKey === 'openai-compatible'
}

function printQuickstartChoices() {
  console.log('Choose a setup profile:\n')
  getQuickstartProviderChoices().forEach((choice, index) => {
...
async function promptForProviderConfig(rl, config, providerKey) {
  if (!sources[providerKey]) return
  ...
  if (isConfigurableModelProvider(providerKey)) {
    console.log('  note: configure the provider base URL and model ID after the API key prompt')
  }
  ...
  if (isConfigurableModelProvider(providerKey)) {
    ...
    const defaultsMeta = getProviderSetupMeta(providerKey)
    const defaults = {
      baseUrl: defaultsMeta.defaultBaseUrl || '',
      modelId: defaultsMeta.defaultModelId || '',
    }
    console.log(`  current base URL: ${currentBaseUrl || defaults.baseUrl || '(none)'}`)
    ...
    else if (defaults.baseUrl && !currentBaseUrl) config.providers[providerKey].baseUrl = defaults.baseUrl

    console.log(`  current model ID: ${currentModelId || defaults.modelId || '(none)'}`)
    ...
    else if (defaults.modelId && !currentModelId) config.providers[providerKey].modelId = defaults.modelId
...
function ensureOpenClawModelLane(config, providerKey, modelId = 'auto-fastest') {
  if (!config.agents) config.agents = {}
  if (!config.agents.defaults) config.agents.defaults = {}
  if (!config.agents.defaults.models) config.agents.defaults.models = {}

  const laneKey = `${providerKey}/${modelId}`
  if (!config.agents.defaults.models[laneKey]) {
    config.agents.defaults.models[laneKey] = {}
  }

  if (!config.agents.defaults.model) config.agents.defaults.model = {}
  if (config.agents.defaults.model.primary === 'modelrelay/auto-fastest') {
    config.agents.defaults.model.primary = laneKey
  }
}
...
function configureOpenClaw(port) {
  ...
  config.models.providers.modelfoundry = buildOpenClawProviderConfig(port)

  ensureOpenClawModelLane(config, 'modelfoundry')
...
function printIntegrationSnippets(port) {
  ...
  const openclawSnippet = [
    ...
    '      "models": { "modelfoundry/auto-fastest": {} }',
    '    }',
    '  }',
    '}',
  ].join('\n')
...
export async function runOnboard(port = 7352) {
  ...
  console.log('\nModelFoundry onboarding')
  console.log('Enter an API key, press Enter to keep existing, or type - to clear.\n')
  printQuickstartChoices()
  const quickstartAnswer = await rl.question('Choose [1-4] (default: 1): ')
  const setupChoice = parseQuickstartProviderChoice(quickstartAnswer)

  if (setupChoice === 'advanced') {
    console.log('\nAdvanced setup selected. Walking the full provider list.\n')
    for (const providerKey of PROVIDER_ORDER) {
      await promptForProviderConfig(rl, config, providerKey)
    }
  } else {
    const meta = getProviderMeta(setupChoice)
    console.log(`\nQuick Start: ${sources[setupChoice]?.name || setupChoice}`)
    console.log(`${meta.summary}`)
    console.log('You can add more providers later in Settings or by rerunning `model-foundry onboard`.\n')
    await promptForProviderConfig(rl, config, setupChoice)
  }

  saveConfig(config)
  console.log('Saved API keys to ~/.model-foundry.json (legacy mirror: ~/.modelrelay.json)')
